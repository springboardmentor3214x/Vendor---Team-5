...
import { Invoice, ProcurementService } from '../../core/services/procurement.service';
...
export class InvoiceDetailComponent {
  ...
  get paymentOptions(): string[] { return this.invoice()?.payment_status === 'Verified' ? ['Approved', 'Rejected'] : this.invoice()?.payment_status === 'Approved' ? ['Paid'] : []; }
  ...
  get canUpdatePayment(): boolean { return this.paymentOptions.length > 0; }
  ...
  private runAction(action: InvoiceAction, call: ReturnType<ProcurementService['getInvoice']>, message: string): void { this.acting.set(action); this.errorMessage.set(''); this.successMessage.set(''); call.subscribe({ next: (invoice) => { this.invoice.set(invoice); this.syncPaymentForm(invoice); this.successMessage.set(message); this.acting.set(null); }, error: (error: unknown) => { this.errorMessage.set(this.readError(error)); this.acting.set(null); } }); }
  private load(invoiceId: number): void { this.procurementService.getInvoice(invoiceId).subscribe({ next: (invoice) => { this.invoice.set(invoice); this.syncPaymentForm(invoice); this.loading.set(false); }, error: (error: unknown) => { this.errorMessage.set(this.readError(error)); this.loading.set(false); } }); }
  private syncPaymentForm(invoice: Invoice): void { this.paymentForm.patchValue({ paymentStatus: invoice.payment_status === 'Approved' ? 'Paid' : 'Approved' }); }
